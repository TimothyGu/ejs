...
/**
 * Copy properties in locals object that are recognized as options to an
 * options object.
 *
...
function cpOptsInLocals(locals, options) {
  if (locals.__expressRender__) {
    optInLocalsWarned = true;
  }
  _OPTS.forEach(function (p) {
    if (typeof locals[p] != 'undefined') {
      if (!(optInLocalsWarned || locals.__expressRender__)) {
  ...
  delete locals.__expressRender__;
}
...
function parse(str, options) {
  var d = options.delimiter || exports.delimiter || _DEFAULT_DELIMITER
    , open = '<' + d
    , close = d + '>'
    , globals = ['__line', '__lines', '__filename', 'include', 'escape', 'rethrow', 'buf']
  ...
  if (false !== options._with) {
    buf += "');";
    try {
      buf = addWith('locals', buf, globals);
    }
    catch(e) {
...
exports.compile = function compile(str, options) {
  ...
  return function (locals) {
    function include(path, includeLocals) {
      ...
      copiedOpts.filename = resolveInclude(path, options.filename);
...
exports.renderFile = function renderFile(path, locals, options, fn) {
  ...
  try {
    str = handleCache(options)(locals);
  }
  catch (e) {
    return fn(e);
  }
