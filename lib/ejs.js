...
/**
 * Whether or not to favor execution speed over code size.
 *
 * This variable is overriden by {@link Options}`.favorSpeed` if it is not
 * `undefined`.
 *
 * @type {Boolean}
 * @public
...
exports.favorSpeed = true;

/**
 * Whether or not to make dynamic inclusion possible
 *
 * This variable is overriden by {@link Options}`.dynInc` if it is not
 * `undefined`.
 *
 * @type {Boolean}
 * @public
...
function parse(str, options) {
  var d = options.delimiter || exports.delimiter || _DEFAULT_DELIMITER
    ...
    , speed = options.favorSpeed !== undefined
            ? options.favorSpeed
            : exports.favorSpeed
    , buf = '  var __output = [];\n';
  ...
  for (var i = 0, len = str.length; i < len; ++i) {
    ...
    if (str.slice(i, open.length + i) == open) {
      ...
      else if (!inText && !speed) {
        buf += "\n  , '";
        inText = true;
      }
      buf += (stri in strMap) ? strMap[stri] : stri;
    }
  }
...
exports.compile = function compile(str, options) {
  ...
  if (options.dynInc === false) {
    return function (locals) {
      return fn(locals, escape, null, rethrow);
    }
  }
  return function (locals) {
